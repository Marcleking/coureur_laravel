...
function lstRessource() {
	...
	$listeRessourceBrute = $prep->fetchAll(PDO::FETCH_ASSOC);
	$listeRessource = arrayRessource();
	foreach($listeRessourceBrute as &$laRessource) {
		//var_dump($laRessource);
		...
		for($j = ($lesHrsChiffre[0]-9)*2; $j <= (($lesHrsChiffre[0]-9)*2 + ($lesHrsChiffre[1] - $lesHrsChiffre[0]) * 2 )-1 ; $j++) { 
			$listeRessource["ressourceVetement"][$laRessource['jour']][$j] = $laRessource['nbEmpVetements'];
			$listeRessource["ressourceChaussure"][$laRessource['jour']][$j] = $laRessource['nbEmpChaussures'];
			$listeRessource["ressourceCaissier"][$laRessource['jour']][$j] = $laRessource['nbEmpCaissier'];
		}
	}
	//var_dump($listeRessource);
...
*/
function arrayRessource() {
	$arraySemaine =  array("ressourceVetement" => array(), "ressourceChaussure" => array(), "ressourceCaissier" => array() );
	...
	for($i = 0; $i < 7; $i++) {
		...
		$arraySemaine["ressourceVetement"][$i] = $arrayJour;
		$arraySemaine["ressourceChaussure"][$i] = $arrayJour;
		$arraySemaine["ressourceCaissier"][$i] = $arrayJour;
	}
...
function divisionParRessource($listRessource) {
	...
	for($i = 0; $i < 7; $i++) {
		for($j = 0;$j < 24; $j++) {
			if($listRessource['ressourceVetement'][$i][$j] != 0)
				$RatioDemiHrs["ratioVetement"][$i][$j]["ratio"]  /= $listRessource['ressourceVetement'][$i][$j];
			
			if($listRessource['ressourceChaussure'][$i][$j] != 0)
				$RatioDemiHrs["ratioChaussure"][$i][$j]["ratio"]  /= $listRessource['ressourceChaussure'][$i][$j];
				
			if($listRessource['ressourceCaissier'][$i][$j] != 0)
				$RatioDemiHrs["ratioCaissier"][$i][$j]["ratio"] /= $listRessource['ressourceCaissier'][$i][$j];
		}
...
function verifierSiRessource($jour, $demi) {
	...
	if($listRessource["ressourceVetement"][$jour][$demi] != 0 && $listRessource["ressourceChaussure"][$jour][$demi] != 0 && $listRessource["ressourceCaissier"][$jour][$demi] != 0) {
		$ressource = true;
	}
